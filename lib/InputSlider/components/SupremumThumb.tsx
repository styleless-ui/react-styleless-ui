...
import { SupremumThumbRoot as SupremumThumbRootSlot } from "../slots";
import Thumb, {
...
const SupremumThumbBase = (props: Props, ref: React.Ref<HTMLDivElement>) => {
  const { label, onMouseDown, onTouchStart, onKeyDown, ...otherProps } = props;
  ...
  if (!ctx) {
    logger(
      ...
      {
        scope: "InputSlider.SupremumThumb",
        type: "error",
      },
    );
  ...
  const labelInfo = getLabelInfo(label, "InputSlider.SupremumThumb", {
    customErrorMessage: [
  ...
  const position = getPositions().supremum;
  const thumbInfo = getThumbsInfo().supremum;
  ...
  return (
    <Thumb
      ...
      data-slot={SupremumThumbRootSlot}
    />
  );
};

const SupremumThumb = componentWithForwardedRef(
  SupremumThumbBase,
  "InputSlider.SupremumThumb",
);

export default SupremumThumb;
