...
describe("TreeView", () => {
  ...
  it("should have the required classNames", async () => {
    const itemClassName: TreeView.ItemProps["className"] = props => {
      const { active, expandable, expanded, selected } = props;
  ...
  it("should have the required aria attributes", async () => {
    ...
    const tree = screen.getByRole("tree");
    const normalItem = screen.getByTestId("1");
    const expandableItem = screen.getByTestId("2");
    const subtree = screen.getByRole("group");
    const innerItem = screen.getByTestId("3");
    ...
    expect(normalItem).not.toHaveAttribute("aria-expanded");
    expect(normalItem).toHaveAttribute("aria-selected", "true");
    expect(normalItem).toHaveAttribute("aria-level", "1");
    expect(normalItem).toHaveAttribute("aria-posinset", "1");
    expect(normalItem).toHaveAttribute("aria-setsize", "2");

    expect(expandableItem).toHaveAttribute("aria-expanded", "true");
    expect(expandableItem).toHaveAttribute("aria-selected", "false");
    expect(expandableItem).toHaveAttribute("aria-level", "1");
    expect(expandableItem).toHaveAttribute("aria-posinset", "2");
    expect(expandableItem).toHaveAttribute("aria-setsize", "2");

    expect(subtree).toHaveAttribute("aria-label", "label");

    expect(innerItem).not.toHaveAttribute("aria-expanded");
    expect(innerItem).toHaveAttribute("aria-selected", "false");
    expect(innerItem).toHaveAttribute("aria-level", "2");
    expect(innerItem).toHaveAttribute("aria-posinset", "1");
    expect(innerItem).toHaveAttribute("aria-setsize", "1");
  });
